...
//    private static final int INF = Integer.MAX_VALUE;   // 最大值
    ...
    public MatrixUDG() {
        ...
        for (int i = 0; i < mVexs.length; i++) {
            ...
            mVexs[i] = readChar();//这里是char的ascll码
        }
     ...
     *     matrix-- 矩阵(数据)
     */
    public MatrixUDG(int[] vexs, int[][] matrix) {
...
//        System.out.println(list);//打乱后

        Integer[] random_array=new Integer[floy.length];//转换为数组
        list.toArray(random_array);
//        Integer[] random_array={9,0,2,4,1,7,3,5,6,8};
//        this.print_random_array(2,random_array);
        //以下计算Omega Ω
        int group_number=6;
        int[][] Omega=new int[floy.length][floy.length/group_number];//2个点一组
//        System.out.println(Omega.length+"  "+Omega[0].length);
        for (int i = 0; i < floy.length; i++) {
            int Omega_j=0;
            for (int j = 0; j < 54; j=j+group_number) {
...
//                Omega[i][Omega_j++]=(path[i][random_array[j]]<path[i][random_array[j+1]])?path[i][random_array[j]]:path[i][random_array[j+1]];
                int temp=2147483647;
                for(int j_group_calc=j;j_group_calc<j+6;j_group_calc++){
                    temp=(floy[i][random_array[j_group_calc]]<temp)?floy[i][random_array[j_group_calc]]:temp;
                }
                Omega[i][Omega_j++]=temp;
            }
        }
...
//            System.out.print(") ");
    ...
    //RE 为Road-Embedding,现在存在对象里称为this.Omega
    //calc_point是计算点的Road_embedding
    //next_a是邻接的第一个点,next_b是邻接的第二个点,point2a_len是点到a的距离
    public int[] calc_point(int next_a,int next_b,int point2a_len){
        int[] point_RE=new int[this.Omega[0].length];
        int point2b_len=this.floy[next_a][next_b]-point2a_len;//a2b_len为floyd的最短路径图
        for (int i = 0; i < this.Omega[0].length; i++) {
            point_RE[i]=Math.min(this.Omega[next_a][i]+point2a_len,this.Omega[next_b][i]+point2b_len);
        }
        return point_RE;
    }
    // 边的结构体
    private static class EData {
        int start; // 边的起点  o_char
        int end;   // 边的终点 o_char
        int weight; // 边的权重

        public EData(int start, int end, int weight) {//0_char
            this.start = start;
            this.end = end;
            this.weight = weight;
        }
